...
$(document).ready(function() {
	$.ajax({
		url: "https://api.myjson.com/bins/368ec?pretty=1",
		dataType: "json",
		success: function(response) {
			$.each(response.apartments, function(i, apartment) {
				var apartmentClass = apartment.city.toLowerCase().replace(" ", "-");
				var listing = "<a href='#' id=" + apartment.id + " class='list-group-item " + apartmentClass + " listings' data-toggle='modal' data-target='#myModal'><h4 class='list-group-item-heading'>" + apartment.description + " / " + apartment.bedrooms + " Bedroom" + " / " + apartment.price + " per month" + "</h4><p class='list-group-item-text'>" + apartment.neighborhood + "</p></a>";
